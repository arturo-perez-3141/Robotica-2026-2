#!/usr/bin/env python3
from sympy import *
# Asegúrate de importar la clase Kinematics del mismo directorio o paquete
# from kinematics import RobotKinematics  <-- Si está en el mismo archivo o carpeta
...
class RobotDynamics():
  def __init__(self):
    pass
  def define_kinematics(self, kinematics):
  ...
  def lagrange_effort_generator(self):
    ...
    #Energía potencial
    # IMPORTANTE: Al ser planar, p_0_Ci en Z será constante (0), por lo que u será constante.
    # La derivada de u respecto a theta será 0. El término gravitacional desaparecerá.
    ...
    for i in range(self.kinematics.samples):
      vals = tau_f( float(self.kinematics.q_m[0, i]),         float(self.kinematics.q_m[1, i]),          float(self.kinematics.q_m[2, i]),
      ...
      self.tau_m[:, i] = Matrix(vals)

  def effort_graph(self):
    fig, ((tau_1_g, tau_2_g, tau_3_g)) = plt.subplots(nrows=1, ncols = 3)
    fig.suptitle("Pares en las juntas (Robot Planar - Sin Gravedad Efectiva)")
  ...
  def redefine_kinematics(self):
    # OJO: Necesitas pasar la clase RobotKinematics importada
    self.kinematics = RobotKinematics()
    self.kinematics.direct_kinematics()
    
    # Importante: xi_fn ahora es X, Y, Theta.
    # Define un punto alcanzable, ej: x=0.5, y=0.3
    self.kinematics.trajectory_generator(q_in=[0.1, 0.1, 0.1], xi_fn = [0.5, 0.3, 1.57]) 
